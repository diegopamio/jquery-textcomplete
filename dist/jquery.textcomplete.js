...
if (typeof rangy.saveSelection === 'undefined') {
    throw new Error('jQuery.textcomplete requires rangy-saverestore');
}
...
+function ($) {
    ...
    $.extend(Completer.prototype, {
        ...
        trigger: function (text, skipUnchangedTerm) {
            if (!this.adapter.savedSel) {
                this.adapter.savedSel = rangy.saveSelection();
            }
            ...
            if (searchQuery.length) {
                ...
                if (skipUnchangedTerm && this._term === term && this._text === text) { return; }
                this._term = term;
                this._text = text;
                this._search.apply(this, searchQuery);
                this.$el.focus();
                console.log(this.dropdown.data[0].value.text);
                if (this.option.autoselectExactMatch &&
                    (this.dropdown.data.length === 1 || (this.dropdown.data.length > 1 && term.toLowerCase() === this.dropdown.data[0].value.text.toLowerCase())) &&
                    term.toLowerCase().indexOf(this.dropdown.data[0].value.text.toLowerCase()) === 0
                    ) {
                    var onlyOption = this.dropdown.data[0];

                    this.adapter.select(onlyOption.value, onlyOption.strategy);
                }
            } else {
                this._term = null;
        ...
        getTerm: function() {
            var text = this.adapter.getContentFromHeadToCaret();
            var searchQuery = this._extractSearchQuery(text);
            if (searchQuery.length) {
...
+function ($) {
    ...
    $.extend(Dropdown.prototype, {
        ...
        _onClick: function (e) {
            ...
            var oldData = _.cloneDeep(this.data);
...
//      if (this.completer.getTerm() !== '') {
            pre = pre.replace(strategy.match, newSubstr);
//      } else {
//        pre = pre + pre.replace(strategy.match, newSubstr);
//      }
            ...
            //setTimeout(function () {
            this.completer.trigger(this.getContentFromHeadToCaret(), false);
            //}, 300);
        ...
        _getCaretRelativePosition: function () {
            ...
            position.lineHeight = $node.height() || 16;
            position.top += position.lineHeight - this.$el.offset().top + 7;
        ...
        getContentFromHeadToCaret: function (onlyText) {
            if (this.option.useInnerHTML && !onlyText) {
                ...
                var content = innerHTML.substr(0, innerHTML.indexOf('<span id="' + this.savedSel.rangeInfos[0].markerId + '"'));
                console.log(content);
                return content;
//            }
            } else {
                ...
                selection.selectNodeContents(range.startContainer);
                console.log("SC:" + selection.toString().substring(0, range.startOffset));
        ...
        getContentFromCaretEnd: function (onlyText) {
            if (this.option.useInnerHTML && !onlyText) {
                var content = this.el.innerHTML.substr(this.el.innerHTML.indexOf('<span id="' + this.savedSel.rangeInfos[0].markerId))
                return content;
            } else {
                ...
                selection = range.cloneRange();
                selection.selectNodeContents(range.startContainer);
                console.log("CE:" + selection.toString().substring(0, range.startOffset));
                return selection.toString().substring(0, range.startOffset);
            }
